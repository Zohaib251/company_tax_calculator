...
const fbrCodeMapping = {
    ...
    'C86': { field: 'EXPENDITURE ATTRIBUTABLE TO NON-BUSINESS INCOME', code: '3218' },
...
function initializeRowMapping() {
    ...
    const calcRows = [
        ...
        101, 102, 103, 104, 105, 106, 108, 110, 111, 112, 113,
...
const tableStructure = [
    ...
    { type: 'input', displayRow: 69, description: 'PROVISION FOR DOUBTFUL DEBTS' },
    { type: 'input', displayRow: 70, description: 'PROVISION FOR OBSOLETE STOCKS' },
    { type: 'input', displayRow: 71, description: 'PROVISION FOR DIMINUTION IN VALUE OF INVESTMENT' },
    { type: 'input', displayRow: 72, description: 'PROVISION FOR RESERVES / FUNDS' },
    { type: 'input', displayRow: 73, description: 'CESS / RATE / TAX LEVIED ON PROFITS' },
    { type: 'input', displayRow: 74, description: 'AMOUNT OF TAX DEDUCTED AT SOURCE' },
    { type: 'input', displayRow: 75, description: 'PAYMENTS LIABLE TO TAX NOT DEDUCTED' },
    { type: 'input', displayRow: 76, description: 'ENTERTAINMENT EXPENDITURE' },
    { type: 'input', displayRow: 77, description: 'CONTRIBUTIONS TO UNRECOGNIZED FUNDS' },
    ...
    { type: 'input', displayRow: 83, description: 'EXPENDITURE NOT THROUGH DIGITAL MODE' },
    ...
    { type: 'input', displayRow: 90, description: 'UTILITY BILLS EXCEEDING LIMIT' },
    { type: 'input', displayRow: 91, description: 'PROFIT ON DEBIT INADMISSIBLE U/S 106A' },
    { type: 'input', displayRow: 92, description: 'PRE-COMMENCEMENT EXPENDITURE' },
    ...
    { type: 'input', displayRow: 112, description: 'TAX DEPRECIATION / INITIAL ALLOWANCE' },
    { type: 'input', displayRow: 113, description: 'PRE-COMMENCEMENT EXPENDITURE' },
...
function generateFBRTable() {
    console.log('📊 Generating FBR Code Table with D Column...');
    
    // Check if container exists, if not create it
    let fbrContainer = document.getElementById('fbrTableContainer');
    if (!fbrContainer) {
        fbrContainer = document.createElement('div');
        fbrContainer.id = 'fbrTableContainer';
        fbrContainer.style.cssText = `
            margin-top: 30px;
            padding: 20px;
            background: #f8f9fa;
            border-radius: 8px;
        ...
        if (resultsPanel) {
            resultsPanel.parentNode.insertBefore(fbrContainer, resultsPanel.nextSibling);
        } else {
            document.querySelector('.container').appendChild(fbrContainer);
        }
    }
    
    // Create table HTML with 5 columns
    let tableHTML = `
        <h3 style="color: #2c3e50; margin-bottom: 15px;">📋 FBR Code Summary</h3>
            ...
            ">
                <thead>
                    <tr style="background: #2c3e50; color: white;">
                        <th style="padding: 10px; text-align: center; border: 1px solid #ddd; width: 5%;">#</th>
                        <th style="padding: 10px; text-align: left; border: 1px solid #ddd; width: 35%;">Field Name</th>
                        <th style="padding: 10px; text-align: center; border: 1px solid #ddd; width: 15%;">FBR Code</th>
                        <th style="padding: 10px; text-align: right; border: 1px solid #ddd; width: 20%;">Value (C)</th>
                        <th style="padding: 10px; text-align: right; border: 1px solid #ddd; width: 25%;">AMOUNT EXEMPT FROM TAX/ SUBJECT TO FIXED/FINAL TAX (D)</th>
                    </tr>
                </thead>
                <tbody>
    `;
    ...
    // Get all FBR mapped fields
    for (const [cellId, mapping] of Object.entries(fbrCodeMapping)) {
        // Get C column value
        let valueC = taxEngine.getValue(cellId);
        
        // Get D column value (replace C with D)
        let cellD = cellId.replace('C', 'D');
        let valueD = taxEngine.getValue(cellD);
        
        // Skip if both values are 0 (optional - show all fields)
        ...
        const rowColor = count % 2 === 0 ? '#f8f9fa' : 'white';
        const isTaxRow = cellId.startsWith('E') ? '#fff3e0' : rowColor;
        const isTotalRow = ['C3', 'C13', 'C26', 'C66', 'C126', 'E128'].includes(cellId) ? '#e6f7ff' : isTaxRow;
        
        // Format D value - if 0 show "-"
        const displayD = valueD !== 0 ? formatNumber(valueD) : '-';
        
        tableHTML += `
            <tr style="background: ${isTotalRow};">
                <td style="padding: 8px 10px; border: 1px solid #ddd; text-align: center;">${count}</td>
                <td style="padding: 8px 10px; border: 1px solid #ddd; font-weight: ${mapping.field.includes('TOTAL') || mapping.field.includes('PROFIT') || mapping.field.includes('INCOME') ? 'bold' : 'normal'};">
                    ${mapping.field}
                </td>
                <td style="padding: 8px 10px; border: 1px solid #ddd; text-align: center; font-family: monospace; font-weight: bold; color: #1890ff;">
                    ${mapping.code || '-'}
                </td>
                <td style="padding: 8px 10px; border: 1px solid #ddd; text-align: right; font-family: monospace; color: #333;">
                    ${formatNumber(valueC)}
                </td>
                <td style="padding: 8px 10px; border: 1px solid #ddd; text-align: right; font-family: monospace; color: ${valueD > 0 ? '#52c41a' : '#999'}; font-weight: ${valueD > 0 ? 'bold' : 'normal'};">
                    ${displayD}
                </td>
            </tr>
        `;
    ...
    // Add total row
    tableHTML += `
        <tr style="background: #e6f7ff; font-weight: bold; border-top: 2px solid #1890ff;">
            <td colspan="3" style="padding: 10px; border: 1px solid #ddd; text-align: right; font-size: 14px;">TOTAL</td>
            <td style="padding: 10px; border: 1px solid #ddd; text-align: right; font-family: monospace; font-size: 14px; color: #1890ff;">
                ${formatNumber(totalC)}
            </td>
            <td style="padding: 10px; border: 1px solid #ddd; text-align: right; font-family: monospace; font-size: 14px; color: #52c41a;">
                ${formatNumber(totalD)}
            </td>
        </tr>
    `;
    ...
    tableHTML += `
        ...
        <div style="margin-top: 10px; display: flex; justify-content: space-between; font-size: 11px; color: #999;">
            <span>Total Fields: ${count}</span>
            <span>Total Value (C): Rs. ${formatNumber(totalC)} | Total Exempt (D): Rs. ${formatNumber(totalD)}</span>
            <span style="color: #52c41a;">✅ D = Amount Exempt from Tax / Subject to Fixed/Final Tax</span>
        </div>
    `;
    
    fbrContainer.innerHTML = tableHTML;
    console.log(`✅ FBR Table generated with ${count} fields`);
}
...
function init() {
    ...
    // Create FBR Table container (hidden initially)
    const fbrContainer = document.createElement('div');
    fbrContainer.id = 'fbrTableContainer';
    fbrContainer.style.display = 'none';
    fbrContainer.style.cssText = `
        margin-top: 30px;
        padding: 20px;
        background: #f8f9fa;
    ...
    container.appendChild(fbrContainer);
    ...
    document.getElementById('calculateTax').addEventListener('click', function(event) {
        ...
        if (panel.style.display === 'none') {
            ...
            // Generate and show FBR Table
            generateFBRTable();
            document.getElementById('fbrTableContainer').style.display = 'block';
            ...
            // Hide FBR Table
            document.getElementById('fbrTableContainer').style.display = 'none';
